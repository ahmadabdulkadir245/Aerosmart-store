...
const Products: React.FC<ProductProps> = ({ id, title, price, description, images, category, user_id }) => {
...
// 
  ...
  const handleAddToCart = async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.post('/api/addToCart', { id, user_id, qauntity: 1 });
        ...
        <div className='relative flex flex-col bg-white z-30 shadow-xl rounded-sm text-gray-500 overflow-hidden transition delay-100 ease-in-out'>
          <Link href={`/products/${id}`}>
            <div className='relative w-full h-[160px]'>
              <Image src={images[0].url} alt={images[0].url} layout="fill" objectFit="contain" placeholder="blur" loading="lazy" />
            </div>
          </Link>
          ...
          <div className='font-primary line-clamp-2 lg:line-clamp-3 px-2 text-xs h-[30px] lg:h-[50px] text-gray-800'>
            ...
            {description? description : <p>this is the descripton of the product, it is strong, durable, and last long.</p>}
          </div>
