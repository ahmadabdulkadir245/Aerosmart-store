...
interface Product {
  id: number;
  title: string;
  images: string[]; // Define images as an array of strings (image URLs)
  price: number;
}
          ...
          {products.map((product) => (
            <SwiperSlide key={product.id} onClick={() => router.push(`/products/${product.id}`)}>
              <div className="relative h-[150px] w-[200px] m-auto rounded-md overflow-hidden shadow-sm">
                <Image src={product.images[0]} alt={product.title} layout="fill" objectFit="cover" priority />
              </div>
              ...
              <div className="capitalize text-xs pt-1  w-[90%] ">
